...
const seasonData = {
  ...
  'high-stakes': {
    nameKey: 'ourRobot.seasons.highStakes.name',
    year: '2024-2025',
    ...
    images: {
      left: { src: '/models/HiS/1.png', altKey: 'ourRobot.faceView', titleKey: 'ourRobot.faceView' },
      right: { src: '/models/HiS/2.png', altKey: 'ourRobot.backView', titleKey: 'ourRobot.backView' }
    },
    specs: {
      dimensions: '15" × 13.5" × 18"',
      weightKey: 'ourRobot.specs.highStakesWeight',
      driveSystem: '6-motor 450RPM 3.25" Omnis',
  ...
  return (
    <Html center>
      <div className="text-center">
        ...
        <p className="text-gray-600">{t('ourRobot.loadingModel')}</p>
      </div>
    </Html>
  );
}
...
function SeasonDropdown({ selectedSeason, onSeasonChange }: { 
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      >
        {t(seasonData[selectedSeason as keyof typeof seasonData]?.nameKey)} - {seasonData[selectedSeason as keyof typeof seasonData]?.year}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6 h-full">
            <div className="bg-white rounded-2xl shadow-xl p-6 h-full flex flex-col">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-xl font-semibold text-gray-800">{t('ourRobot.chassisCad')}</h3>
              </div>
              ...
              <div className="flex-1 w-full rounded-xl overflow-hidden bg-[#2a2a2a] relative">
                ...
                {isLoading && (
                  <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-90">
                    <div className="text-center">
                      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
                      <p className="text-gray-600">{t('ourRobot.initializingViewer')}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6 h-full">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="rounded-lg overflow-hidden mb-3 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => openLightbox(currentSeason.images.left.src)}>
                  <img
                    ...
                    alt={t(currentSeason.images.left.altKey)}
                    className="w-full h-full object-cover"
                  />
                </div>
                <h4 className="font-semibold text-gray-800 mb-2">{t(currentSeason.images.left.titleKey)}</h4>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="rounded-lg overflow-hidden mb-3 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => openLightbox(currentSeason.images.right.src)}>
                  <img
                    ...
                    alt={t(currentSeason.images.right.altKey)}
                    className="w-full h-full object-cover"
                  />
                </div>
                <h4 className="font-semibold text-gray-800 mb-2">{t(currentSeason.images.right.titleKey)}</h4>
                {/* <p className="text-sm text-gray-600">
           ...
           <div className="bg-white rounded-xl shadow-lg p-6">
             <h4 className="font-semibold text-gray-800 mb-4">{t('ourRobot.technicalSpecifications')}</h4>
             <div className="grid grid-cols-2 gap-4 text-sm">
               <div>
                 <span className="text-gray-600">{t('ourRobot.dimensions')}:</span>
                 <span className="block font-medium">{currentSeason.specs.dimensions}</span>
               </div>
               <div>
                 <span className="text-gray-600">{t('ourRobot.weight')}:</span>
                 <span className="block font-medium">{t(currentSeason.specs.weightKey)}</span>
               </div>
               <div>
                 <span className="text-gray-600">{t('ourRobot.driveSystem')}:</span>
                 <span className="block font-medium">{currentSeason.specs.driveSystem}</span>
               </div>
               <div>
                 <span className="text-gray-600">{t('ourRobot.motors')}:</span>
